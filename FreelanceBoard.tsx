...
import { MoreHorizontal, Plus, X, Trash2, Check, Clock, Circle } from 'lucide-react';
...
const FreelanceBoard: React.FC<FreelanceBoardProps> = ({ lang, tasks, setTasks }) => {
  ...
  const handleSave = () => {
      ...
      if (editingTask) {
          ...
          setTasks(prev => prev.map(task => 
              ...
              ? { ...task, title, energyCost: energy, context, status, completedAt }
              : task
          ));
      } else {
          // Create
                ...
                {freelanceTasks.filter(t => t.status === col.id).map(task => (
                    ...
                    >
                        ...
                        <div className="flex items-center justify-between">
                             ...
                             <span className="text-white/40 text-xs font-mono" dir="ltr">${task.energyCost * 150}</span>
                        </div>
                    </GlassCard>
                ))}
                {col.id === 'todo' && (
                                      ...
                                      {[1, 2, 3].map((level) => (
                                          <button
                                            ...
                                            className={`flex-1 h-8 rounded-lg border flex items-center justify-center transition-all ${
                                                energy === level ? 'bg-blue-500 border-blue-400 text-white' : 'bg-white/5 border-white/10 text-white/40'
                                            }`}
                                          >
                                              {level}
                                          </button>
